...
export default function Page() {
  ...
  const getAllTaskMembers = () => {
    ...
    return Array.from(memberIds).slice(0, 4);
  };
  ...
  return (
    <div className={`flex h-screen w-full overflow-hidden ${actualTheme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
      <Sidebar currentView={"calendar"} onViewChange={setCurrentView} />

      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 p-3 sm:p-4 border-b shrink-0 min-w-0 ${actualTheme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="relative flex-1 w-full max-w-md min-w-0">
                <Search className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 ${actualTheme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`} />
                <Input
                    placeholder="Find Something"
                    className={`pl-11 h-10 w-full ${actualTheme === 'dark' ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-900'}`}
                    style={{
                      borderRadius: '24px',
            ...
            <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto">
                <span className={`text-sm ${actualTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{getLastEditedTime()}</span>
                ...
                <Button
                    ...
                    variant="outline"
                    className={`rounded-xl px-2 sm:px-3 py-2 ${actualTheme === 'dark' ? 'border-gray-600 bg-gray-800 text-gray-100' : 'border-gray-300 bg-white text-gray-900'}`}
                >
        ...
        <div className="flex-1 flex overflow-hidden">
            <main className={`flex-1 flex flex-col p-6 overflow-hidden ${actualTheme === 'dark' ? 'bg-gray-800' : 'bg-gray-50'}`}>
                {/* Sub Header */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-4 mb-6 shrink-0">
                    <div>
                        <h1 className={`text-xl sm:text-2xl font-bold ${actualTheme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>Scheduled Task</h1>
                        <p className={`text-xs sm:text-sm ${actualTheme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{todayFormatted}</p>
                    </div>
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        <Button
                            variant="outline"
                            className={`font-semibold rounded-lg flex-1 sm:flex-none ${actualTheme === 'dark' ? 'border-gray-600 bg-gray-700 text-gray-100' : 'border-gray-300 bg-white text-gray-900'}`}
            ...
            {currentMonth} {currentYear}
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <Button variant="outline" className={`text-sm h-9 ${actualTheme === 'dark' ? 'bg-gray-700 text-gray-100' : 'bg-white'}`} onClick={() => setViewDate(new Date())}>{currentDay} {currentMonth} {currentYear}</Button>
                    ...
                    <div className="grid min-w-[800px]" style={{ 
                        ...
                        {timeSlots.map((hour, index) => (
                            <div key={hour} className="contents">
                                <div className={`text-right pr-4 text-xs font-semibold ${actualTheme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`} style={{
                                    gridColumn: 1,
                                    gridRow: index + 1,
                                    display: 'flex',
                                ...
                                {Array.from({ length: 7 }).map((_, dayIndex) => (
                                    <div key={dayIndex} className={`border-t border-l ${actualTheme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`} style={{ 
                        ...
                        {/* Task Events */}
                        {loading ? (
                          <div className={`col-span-8 row-span-18 flex items-center justify-center ${actualTheme === 'dark' ? 'text-gray-400' : 'text-muted-foreground'}`}>
                            Loading events...
                          </div>
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  margin: 6,
                                  background: task.backgroundColor || 'hsl(var(--primary) / 0.12)',
                                  borderRadius: 16,
                                  padding: '16px',
                                  display: 'flex',
                              ...
                              >
                                <div style={{ fontWeight: 600, fontSize: 15, color: 'hsl(var(--foreground))', lineHeight: 1.3 }}>
                                  {task.title}
                                </div>
                                
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>
                                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                                    <path d="M7 3.5V7L9 9" stroke="hsl(var(--muted-foreground))" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                    <circle cx="7" cy="7" r="5.5" stroke="hsl(var(--muted-foreground))" strokeWidth="1.5"/>
                                  </svg>
                                ...
                                {task.description && (
                                  <div style={{ fontSize: 12, color: 'hsl(var(--muted-foreground))', lineHeight: 1.4, marginTop: -4, maxHeight: 40, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                    {task.description}
                                  </div>
                                )}
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    ...
                                    {task.fullEvent?.owner === currentUserId && (
                                      <span style={{ fontSize: 10, color: 'hsl(var(--primary))', fontWeight: 600 }}>
                                        Owner
                                      </span>
                                    )}
                                  </div>
                                    ...
                                    {isMenuOpen && (
                                      <div
                                        style={{
                                          ...
                                          border: actualTheme === 'dark' ? '1px solid #374151' : '1px solid #E0E0E0',
                                          zIndex: 10,
                                          padding: '8px 0',
                                          display: 'flex',
                                          flexDirection: 'column',
                                      ...
                                      >
                                        <button
                                          style={{
                                            background: 'none',
                                            border: 'none',
                                            color: actualTheme === 'dark' ? '#F3F4F6' : '#222',
                                            fontWeight: 500,
                                            fontSize: 15,
                                            padding: '8px 16px',
                                            textAlign: 'left',
      ...
      {showCalendarPopup && (
                ...
                >
                    <div
                        style={{
                            borderRadius: 24,
                            background: actualTheme === 'dark' ? '#1F2937' : 'hsl(var(--card))',
                            color: actualTheme === 'dark' ? '#F3F4F6' : 'hsl(var(--foreground))',
                    ...
                    >
                        ...
                        <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
                            <button style={{ background: actualTheme === 'dark' ? '#374151' : '#FFF', border: 'none', cursor: 'pointer', padding: 0, borderRadius: '50%' }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="49" height="33" viewBox="0 0 49 33" fill="none">
                                  <g filter="url(#filter0_d_3313_7451)">
                                    <path d="M30.3333 16.3333L37 23M37 23V17.4444M37 23H31.4444M30.3333 9.66667L37 3M37 3V8.55556M37 3H31.4444M17 17.4444V23M17 23H22.5556M17 23L23.6667 16.3333M17 8.55556V3M17 3H22.5556M17 3L23.6667 9.66667" stroke={actualTheme === 'dark' ? '#F3F4F6' : 'black'} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" shapeRendering="crispEdges"/>
                                  </g>
                                  <defs>
                                    <filter id="filter0_d_3313_7451" x="0.200012" y="-11.8008" width="53.6" height="53.6016" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', marginBottom: 8 }}>
                            ...
                            <span style={{ fontWeight: 700, fontSize: 22, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', flex: 1, textAlign: 'center' }}>{calendarMonth} {calendarYear}</span>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0, width: '100%', marginBottom: 16 }}>
                            {calendarDays.map((dayInfo, idx) => (
                                <span
                                  key={idx}
                                  style={{
                                    color: dayInfo.isCurrentMonth ? (actualTheme === 'dark' ? '#F3F4F6' : '#222') : (actualTheme === 'dark' ? '#6B7280' : '#C7C7C7'),
                                    fontWeight: 500,
                                    fontSize: 16,
                                    width: 32,
                                    height: 32,
                                  ...
                                  onMouseEnter={(e) => {
                                    if (dayInfo.isCurrentMonth) {
                                      e.currentTarget.style.background = actualTheme === 'dark' ? '#374151' : '#EFEDFA';
                                    }
                                  }}
                                  onMouseLeave={(e) => {
                        ...
                        <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                            <button onClick={() => setShowCalendarPopup(false)} style={{ background: 'none', border: 'none', color: actualTheme === 'dark' ? '#F3F4F6' : '#222', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>Cancel</button>
      ...
      {showCreateTask && (
                ...
                >
                    ...
                    >
                        {/* Close icon */}
                        <button onClick={() => setShowCreateTask(false)} style={{ position: 'absolute', top: 24, right: 24, background: 'none', border: 'none', fontSize: 22, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', cursor: 'pointer' }}>&times;</button>
                        <div style={{ fontWeight: 600, fontSize: 18, color: actualTheme === 'dark' ? '#9CA3AF' : '#7B8794', marginBottom: 8 }}>Create Task</div>
                        <input
                          type="text"
                          placeholder="Task Title"
                          value={newTask.title}
                          onChange={(e) => setNewTask({...newTask, title: e.target.value})}
                          style={{ width: '100%', fontWeight: 700, fontSize: 22, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', border: 'none', outline: 'none', marginBottom: 8, background: 'transparent' }}
                          className={actualTheme === 'dark' ? 'placeholder-gray-500' : ''}
                        />
                        
                        {/* Project Selection */}
                        <div style={{ width: '100%', marginBottom: 8 }}>
                          <label style={{ display: 'block', fontSize: 14, fontWeight: 500, color: actualTheme === 'dark' ? '#9CA3AF' : '#7B8794', marginBottom: 4 }}>
                            Select Project
                          </label>
                          <select
                            ...
                            style={{
                              ...
                              fontWeight: 500,
                              color: actualTheme === 'dark' ? '#F3F4F6' : '#222',
                              background: actualTheme === 'dark' ? '#374151' : '#FFF',
                              outline: 'none',
                              cursor: 'pointer'
                            }}
                          >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', width: '100%', marginBottom: 8 }}>
                            ...
                            <button
                              ...
                              onMouseEnter={(e) => e.currentTarget.style.background = actualTheme === 'dark' ? '#374151' : '#F3F4F6'}
                              onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
                            >+</button>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
                                {selectedMembers.length === 0 ? (
                                  <span style={{ fontSize: 13, color: actualTheme === 'dark' ? '#9CA3AF' : '#7B8794', fontStyle: 'italic' }}>No members selected</span>
                                ) : (
                                  selectedMembers.slice(0, 5).map((member, idx) => (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        fontSize: 12,
                                        fontWeight: 600,
                                        border: actualTheme === 'dark' ? '2px solid #1F2937' : '2px solid #FFF',
                                        marginLeft: idx > 0 ? -8 : 0,
                                      }}
                                      title={member.name || member.email}
                                    >
                                ...
                                {selectedMembers.length > 5 && (
                                  <div
                                    style={{
                                      ...
                                      height: 32,
                                      borderRadius: '50%',
                                      background: actualTheme === 'dark' ? '#374151' : '#E5E7EB',
                                      display: 'flex',
                                      alignItems: 'center',
                                      justifyContent: 'center',
                                      fontSize: 12,
                                      fontWeight: 600,
                                      color: actualTheme === 'dark' ? '#9CA3AF' : '#7B8794',
                                      marginLeft: -8,
                                    }}
                                  >
                                    +{selectedMembers.length - 5}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', width: '100%', gap: 24, marginBottom: 8 }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 500, fontSize: 15, color: actualTheme === 'dark' ? '#F3F4F6' : '#222' }}>
                                <svg width="18" height="18" viewBox="0 0 22 22" fill="none"><rect x="3" y="6" width="16" height="12" rx="2" stroke={actualTheme === 'dark' ? '#F3F4F6' : '#222'} strokeWidth="2"/><path d="M7 3V6" stroke={actualTheme === 'dark' ? '#F3F4F6' : '#222'} strokeWidth="2" strokeLinecap="round"/><path d="M15 3V6" stroke={actualTheme === 'dark' ? '#F3F4F6' : '#222'} strokeWidth="2" strokeLinecap="round"/></svg>
                                <select value={newTask.day} onChange={(e) => setNewTask({...newTask, day: e.target.value})} style={{ border: 'none', outline: 'none', fontSize: 15, fontWeight: 500, background: 'transparent', color: actualTheme === 'dark' ? '#F3F4F6' : '#222' }}>
                                  {daysOfWeek.map(day => <option key={day} value={day}>{day}</option>)}
                                </select>
                            </label>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 500, fontSize: 15, color: actualTheme === 'dark' ? '#F3F4F6' : '#222' }}>
                                <input
                                  type="time"
                                  value={newTask.startTime}
                                  onChange={(e) => setNewTask({...newTask, startTime: e.target.value})}
                                  style={{ border: 'none', outline: 'none', fontSize: 15, fontWeight: 500, background: 'transparent', color: actualTheme === 'dark' ? '#F3F4F6' : '#222' }}
                                />
                                <span>-</span>
                                <input
                                  type="time"
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', width: '100%', gap: 12, marginBottom: 8 }}>
                            <span style={{ fontWeight: 500, fontSize: 15, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', marginRight: 8 }}>Label</span>
                            <span 
                        ...
                        <textarea
                          ...
                          style={{ width: '100%', minHeight: 160, borderRadius: 12, border: actualTheme === 'dark' ? '1px solid #374151' : '1px solid #E0E0E0', padding: 12, fontSize: 14, fontWeight: 500, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', background: actualTheme === 'dark' ? '#374151' : '#FFF', marginBottom: 12, resize: 'none', outline: 'none' }}
                          className={actualTheme === 'dark' ? 'placeholder-gray-500' : ''}
                        />
                        <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                            ...
                            }} style={{ background: actualTheme === 'dark' ? '#374151' : '#FFF', color: actualTheme === 'dark' ? '#F3F4F6' : '#222', fontWeight: 600, fontSize: 15, border: actualTheme === 'dark' ? '1.5px solid #6B7280' : '1.5px solid #B39DDB', borderRadius: 12, padding: '10px 24px', cursor: 'pointer' }}>Discard</button>
      ...
      {showTeamInfo && selectedEvent && (
        ...
        >
          ...
          >
            ...
            {/* Task Info */}
            <div>
              <h2 style={{ fontSize: 24, fontWeight: 700, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', marginBottom: 8 }}>
                {selectedEvent.title}
              </h2>
              <p style={{ fontSize: 14, color: actualTheme === 'dark' ? '#9CA3AF' : '#666', marginBottom: 16 }}>
            ...
            {/* Team Info */}
            <div style={{ borderTop: actualTheme === 'dark' ? '1px solid #374151' : '1px solid #E0E0E0', paddingTop: 24 }}>
              <h3 style={{ fontSize: 18, fontWeight: 600, color: actualTheme === 'dark' ? '#F3F4F6' : '#222', marginBottom: 16 }}>
                Team Information
              </h3>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 14, color: actualTheme === 'dark' ? '#9CA3AF' : '#666', fontWeight: 500 }}>Owner:</span>
                  <span style={{ fontSize: 14, color: actualTheme === 'dark' ? '#E5E7EB' : '#222', fontWeight: 600 }}>
                    {selectedEvent.owner === currentUserId ? 'You' : 'User ID: ' + selectedEvent.owner}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 14, color: actualTheme === 'dark' ? '#9CA3AF' : '#666', fontWeight: 500 }}>Admin:</span>
                  <span style={{ fontSize: 14, color: actualTheme === 'dark' ? '#E5E7EB' : '#222', fontWeight: 600 }}>
              ...
              {/* Team Members */}
              <div>
                <div style={{ fontSize: 14, color: actualTheme === 'dark' ? '#9CA3AF' : '#666', fontWeight: 500, marginBottom: 12 }}>
                  ...
                  {selectedEvent.members && selectedEvent.members.map((member: string, idx: number) => (
                    <div
                      ...
                      style={{
                        ...
                        gap: 8,
                        padding: '8px 12px',
                        background: actualTheme === 'dark' ? '#374151' : '#F5F5F5',
                        borderRadius: 8,
                      }}
                    >
                      <Image
                      ...
                      />
                      <span style={{ fontSize: 13, color: actualTheme === 'dark' ? '#E5E7EB' : '#222', fontWeight: 500 }}>
                        Member {idx + 1}
                      </span>
                    </div>
                  ))}
              ...
              {selectedEvent.owner === currentUserId && (
                <div style={{ marginTop: 20, paddingTop: 20, borderTop: actualTheme === 'dark' ? '1px solid #374151' : '1px solid #E0E0E0' }}>
                  <Button
                    onClick={() => {
      ...
      {showAllTask && (
        <div
          style={{
            ...
            height: '100vh',
            zIndex: 1002,
            background: actualTheme === 'dark' ? '#111827' : '#FFF',
          }}
        >
